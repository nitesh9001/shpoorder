...
class CouponList extends Component {
  state = {
    ...
    CouponLists: [
      {
        id: 1,
        pincodeApplicableFor: "360001",
        couponName: "20 % discount on Edible Oil",
        description:
          "1. Valid for first time user only 2. Max. 200 Rs of discount can be availed. 3. Minimum Amount should be of 1000 Rs.",
        couponCode: "EDOI20@MF",
        couponCategory: "Oil and Foods",
        sellerId: "AhmSI38002712345",
        maxUsers: "15",
        minAmountofPurchase: "3216",
        maxAmountofPurchase: "10000",
        mindiscountAmount: "500",
        maxdiscountAmount: "1500",
        validFrom: "2020-09-15",
        validTo: "2020-11-27",
        status: "Active",
      },
    ],
    ...
    const columns = [
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.minAmountofPurchaseFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (
                v[0] &&
                v[1] &&
                !this.state.minAmountofPurchaseFilter
              ) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              } else if (v[0]) {
                return `Min: ${v[0]}`;
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.maxAmountofPurchaseFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (
                v[0] &&
                v[1] &&
                !this.state.maxAmountofPurchaseFilter
              ) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              } else if (v[0]) {
                return `Min: ${v[0]}`;
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.mindiscountAmountFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (v[0] && v[1] && !this.state.mindiscountAmountFilter) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              } else if (v[0]) {
                return `Min: ${v[0]}`;
              } else if (v[1]) {
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.maxdiscountAmountFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (v[0] && v[1] && !this.state.maxdiscountAmountFilter) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              } else if (v[0]) {
                return `Min: ${v[0]}`;
              } else if (v[1]) {
          ...
          filterOptions: {
            logic(validFrom, filters) {
              if (filters[0] && filters[1]) {
                ...
                return validFrom < filters[0] || validFrom > filters[1];
              } else if (filters[0]) {
                return validFrom < filters[0];
              } else if (filters[1]) {
                return validFrom > filters[1];
              }
              return false;
            },
            display: (filterList, onChange, index, column) => (
      ...
      },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.validToFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (v[0] && v[1] && !this.state.validToFilter) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              } else if (v[0]) {
                return `Min: ${v[0]}`;
              } else if (v[1]) {
          ...
          filterOptions: {
            logic(validTo, filters) {
              if (filters[0] && filters[1]) {
                ...
                //  var arrEndDate = endDate.split('/');
                //  var date2 = new Date(arrEndDate[2], arrEndDate[1], arrEndDate[0])
                return validTo < filters[0] || validTo > filters[1];
              } else if (filters[0]) {
                return validTo < filters[0];
              } else if (filters[1]) {
                return validTo > filters[1];
              }
              return false;
            },
            display: (filterList, onChange, index, column) => (
