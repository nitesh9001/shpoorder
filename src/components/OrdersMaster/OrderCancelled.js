    ...
    orders_list: [
      {
        ...
        dateOfCancellation: "2020-10-27",
        reasonofCancellation: "payment failed",
    ...
    const columns = [
      ...
      // },
      {
        ...
        options: {
          ...
          customFilterListOptions: {
            render: (v) => {
              if (v[0] && v[1] && this.state.dateOfCancellationFilter) {
                return [`Min: ${v[0]}`, `Max: ${v[1]}`];
              } else if (v[0] && v[1] && !this.state.dateOfCancellationFilter) {
                return `Min: ${v[0]}, Max: ${v[1]}`;
              }
              return [];
            },
          ...
          filterOptions: {
            logic(dateOfCancellation, filters) {
              if (filters[0] && filters[1]) {
                // console.log(dateOfCompletion,filters)
                //   var compDate = dateOfCompletion
                //   var startDate = filters[0];
                ...
                //  var date2 = new Date(arrEndDate[2], arrEndDate[1], arrEndDate[0])
                return (
                  dateOfCancellation < filters[0] ||
                  dateOfCancellation > filters[1]
                );
              } else if (filters[0]) {
                return dateOfCancellation < filters[0];
              } else if (filters[1]) {
                return dateOfCancellation > filters[1];
              }
              return false;
            },
            display: (filterList, onChange, index, column) => (
      ...
      },
      {
        ...
        options: {
          ...
          filterOptions: {
            ...
            display: (filterList, onChange, index, column) => (
              <div>
                <FormLabel style={{ fontSize: "12px" }}>Total Price</FormLabel>
                <FormGroup row>
                  <TextField
                    label="min"
                    value={filterList[index][0] || ""}
...
export default connect(mapStateToProps, {})(OrderCancelled);
