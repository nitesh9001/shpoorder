...
class CancellationPolicyAdd extends React.Component {
  state = {
  ...
  onSave = () => {
    ...
    fetch(Constant.getAPI() + "/metadata/add/cancellationPolicy", {
      method: "post",
      headers: {
      ...
      .then(function (json) {
        if (json.status == true) {
          Swal.fire("Updated !", "Cancellation Policy has been Updated", "success");
  ...
  getRefundPolicy = () => {
    ...
    fetch(Constant.getAPI() + "/metadata/view/cancellationPolicy", {
      method: "post",
      headers: {
        "Content-Type": "application/json",
